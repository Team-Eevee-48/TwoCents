...
feedbackController.getAll = (req, res, next) => {
  ...
  const queryString = `
        ...
        FROM feedback
        LEFT JOIN (
            SELECT COUNT(*) as votes, feedback_id
            FROM votes
            GROUP BY feedback_id
            ) as votes ON votes.feedback_id=feedback._id
        ORDER BY _id DESC
        LIMIT 20;
    `;
...
feedbackController.create = (req, res, next) => {
  ...
  db.query(query, [title, description, category, user_id], (err, response) => {
    if (err)
      return next({
  ...
  db.query(query1, [itemId, user_id], (err, response) => {
    ...
    if (response.rows[0]) {
      ...
      const query2 = `
                ...
                VALUES ($1, $2);
            `;
      db.query(query2, [itemId, user_id], (err, response) => {
        if (err) {
          return next({
          ...
          const query3 = `
                            ...
                            WHERE feedback_id=$1;
                        `;
          db.query(query3, [itemId], (err, response) => {
            if (err)
              return next({
            ...
            if (response.rows[0]) {
              res.locals.votes = response.rows[0].count;
              return next();
            } else {
              return next({
